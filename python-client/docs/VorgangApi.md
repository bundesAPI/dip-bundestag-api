...
with dip_bundestag.ApiClient(configuration) as api_client:
    ...
    try:
        # Liste aller Vorgänge
        api_response = api_instance.vorgang(format=format, cursor=cursor, f_id=f_id, f_datum_start=f_datum_start, f_datum_end=f_datum_end, f_plenarprotokoll=f_plenarprotokoll)
        pprint(api_response)
...
with dip_bundestag.ApiClient(configuration) as api_client:
    ...
    try:
        # Metadaten zu Vorgang
        api_response = api_instance.vorgang_id(id, format=format)
        pprint(api_response)
    except dip_bundestag.ApiException as e:
        print("Exception when calling VorgangApi->vorgang_id: %s\n" % e)
```
